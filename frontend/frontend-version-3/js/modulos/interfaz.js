...
const app = {
    ...
    async navigateTo(page, context = null) {
        ...
        if (previousPage === 'examen' && page !== 'examen') {
            this.hideMathKeyboard(true);
        }
            ...
            <div class="page active">
                <div class="flex justify-between items-center mb-4 flex-wrap gap-3">
                     ...
                     <div class="flex items-center gap-3">
                        <button id="toggle-math-keyboard" onclick="app.toggleMathKeyboard()" class="math-keyboard-button flex items-center gap-2 px-3 py-2 bg-primary text-white rounded-lg shadow-md hover:bg-primary-focus transition-all">
                ...
                <div id="math-keyboard-panel" class="math-keyboard-panel hidden">
                    <div class="math-keyboard-header">
                        <span class="font-semibold">Teclado matemático</span>
                        <button class="keyboard-close" onclick="app.hideMathKeyboard()">✕</button>
                    </div>
                    <p class="text-sm text-slate-500 dark:text-slate-300">Pulsa el botón de teclado para escribir fórmulas. Se cerrará automáticamente al salir del examen.</p>
                </div>
    ...
    configureMathKeyboard() {
        ...
        this.state.mathKeyboardVisible = false;
        const panel = document.getElementById('math-keyboard-panel');
        if (panel) panel.classList.add('hidden');
    },

    toggleMathKeyboard() {
        if (this.state.mathKeyboardVisible) {
            this.hideMathKeyboard();
        } else {
            this.showMathKeyboard();
        }
    },
    ...
    showMathKeyboard() {
        ...
        this.state.mathKeyboardVisible = true;
        const panel = document.getElementById('math-keyboard-panel');
        if (panel) panel.classList.remove('hidden');
    },

    hideMathKeyboard(force = false) {
