...
const resolveFileUrl = (rawUrl) => {
  ...
  // If it's a local public file (starts with /formularios/ or /recursos_comunidad/), return as is
    ...
    <>
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[70] p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-slate-100 rounded-2xl w-full max-w-4xl p-6 relative shadow-2xl">
            <button
              type="button"
            ...
            <div className="mb-4">
              <p className="text-xs uppercase text-slate-500 dark:text-slate-400">{selected.subject || 'General'}</p>
              <h3 className="text-2xl font-bold">{selected.title}</h3>
            </div>

            <div className="bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-white/5 rounded-xl overflow-hidden">
              {previewLoading ? (
                <div className="p-8 text-center text-slate-500 dark:text-slate-400 h-[60vh] flex items-center justify-center">Cargando vista previa…</div>
              ) : previewUrl ? (
                <iframe
                  title="Vista previa"
                ...
                <div className="p-8 text-center text-slate-500 dark:text-slate-400 h-[60vh] flex items-center justify-center">
            ...
            <div className="mt-4 flex flex-col md:flex-row gap-3 md:justify-end">
              {previewUrl ? (
                <button
                  type="button"
                  className="px-4 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-white rounded-lg transition"
              ...
              <button
                ...
                className="px-4 py-2 bg-primary text-white rounded-lg disabled:opacity-60 shadow-lg shadow-primary/20 transition-all active:scale-95"
