    ...
    <>
            ...
            {!isPremium && (
              <div className="mt-4 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-600/80 dark:text-yellow-400/80">Suscríbete para ver y descargar los recursos de la comunidad</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex justify-center mt-8">
            {visibleCount < filtered.length ? (
              <button
                ...
                className="px-6 py-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-semibold rounded-xl transition-colors flex items-center gap-2"
              >
      ...
      {preview && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-2xl w-full max-w-2xl p-6 relative">
            <button className="absolute top-3 right-3 text-slate-400 hover:text-slate-600 dark:hover:text-white" onClick={() => setPreview(null)}>✕</button>
            <p className="text-xs uppercase text-slate-500 dark:text-slate-400 mb-1">{preview.subjectName}</p>
            <h3 className="text-2xl font-bold mb-2">{preview.title}</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Por {preview.author} • {preview.type.toUpperCase()}</p>
            <div className="bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-white/5 rounded-xl p-4 mb-4 text-sm text-slate-800 dark:text-slate-200">
              {(isPremium || purchasedIds.has(preview.id) || preview.free) ? (
                <div className="h-96 w-full flex items-center justify-center bg-slate-200 dark:bg-white/5 rounded-lg">
            ...
            <div className="flex gap-3">
              <button className="w-full py-2 bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-white rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600" onClick={() => setPreview(null)}>Cerrar</button>
            </div>
          </div>
        </div>
      )}
