  ...
  return (
    <div className="page active space-y-6">
      <div>
        ...
        <p className="text-slate-600 dark:text-slate-400">Gestiona los recursos que has publicado.</p>
      </div>
      <button className="py-2 px-4 bg-primary text-white rounded-md w-fit">Crear Nuevo Recurso</button>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {myResources.length ? (
          myResources.map((res) => (
            <div key={res.id} className="glass-effect-light p-6 rounded-2xl flex flex-col gap-3 border border-slate-200 dark:border-white/5 transition-colors">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400">{res.sales || 0} ventas</p>
