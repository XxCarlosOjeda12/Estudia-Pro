...
const ExplorePage = ({ subjects, onAddSubject }) => {
  ...
  return (
    <div className="page active space-y-6">
      <h1 className="text-3xl font-bold text-white">Explorar Materias</h1>
      <div className="glass-effect-light p-6 rounded-2xl space-y-3">
        <div className="relative">
          <input
            ...
            className="w-full p-3 pl-12 bg-slate-900/50 border border-white/10 text-slate-100 placeholder:text-slate-500 rounded-xl focus:ring-2 focus:ring-primary focus:outline-none"
          />
        ...
        <div className="flex flex-wrap gap-2">
          {['Derivadas', 'Matrices', 'Probabilidad'].map((chip) => (
            <button
              ...
              className="px-3 py-1 text-xs rounded-full border border-white/10 text-slate-200 hover:border-primary hover:text-primary"
            >
              {chip}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((subject) => (
          <div key={subject.id} className="glass-effect-light p-6 rounded-2xl border border-white/10 text-slate-100">
            ...
            <button
              type="button"
              className="w-full py-2 px-4 bg-primary text-white rounded-lg hover:bg-primary-focus"
